//! Benchmarks for the runtime archive hot path: digest verification of the
//! pinned Nix tarball and the xz + tar extraction that follows it.
...
/// Deterministic pseudo-random bytes, so every run hashes the same payload.
...
/// Builds an xz-compressed tar archive holding `files` entries of `size` bytes.
...
/// The pinned archive is hashed end to end before it is trusted, so digest
/// throughput is on the critical path of every bootstrap.
...
/// Extraction of many small entries, the shape of the real Nix store archive.
#[divan::bench]
...
/// Extraction dominated by decompressing a single large entry.
#[divan::bench]
